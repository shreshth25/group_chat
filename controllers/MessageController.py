...
class CreateMessage:
    def on_post(self, req, resp):
        try:
            ...
            check_chat_groups = """SELECT chat_groups.id from chat_groups where created_by = :user_id and chat_groups.id= :group_id and chat_groups.is_deleted = 0
                                UNION
                                SELECT g.id FROM group_users
                                join chat_groups as g on g.id = group_users.group_id
                                join users as u on u.id = g.created_by
                                where g.is_deleted = 0 and group_users.user_id = :user_id and g.id= :group_id;"""
